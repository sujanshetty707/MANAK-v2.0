...
import { Globe, Sparkles, ShieldCheck, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
...
export const CheckUrlScreen: React.FC = () => {
  ...
  const [platform, setPlatform] = useState<'amazon' | 'flipkart' | 'blinkit' | 'other'>('other');
  const [urlError, setUrlError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const SUPPORTED = ['amazon.in', 'flipkart.com', 'blinkit.com', 'zepto'];
  ...
  const handleAudit = async () => {
    ...
    try {
      ...
      setUrlError('Failed to parse URL listing. Please ensure backend is reachable.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <main className="flex-1 overflow-y-auto px-4 py-3 space-y-4 hide-scrollbar">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-subtle space-y-3">
          <div>
            ...
            {/* Validation Error */}
            {urlError && (
              <div className="flex items-center gap-1.5 text-[11px] text-red-700 bg-red-50 border border-red-200 rounded-xl px-3 py-2 mt-1.5">
          ...
          {url && (
            <div className="flex items-center justify-between text-xs py-1.5 px-2.5 rounded-xl bg-slate-50 border border-slate-100">
              <span className="text-[11px] text-slate-500">Detected Platform Adapter:</span>
          ...
          >
            ...
            <span>{isLoading ? 'Connecting to Backend API...' : 'Extract & Audit Listing'}</span>
          </button>
        </div>
